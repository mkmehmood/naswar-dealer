/**
 * NativePDF — pure browser PDF generation with zero external dependencies.
 * Uses HTML → hidden iframe → window.print() for download,
 * and Canvas API for image/share output.
 *
 * Public API mirrors the usage patterns from the old jsPDF code:
 *   NativePDF.buildAndDownload(htmlContent, filename)
 *   NativePDF.buildAndShare(htmlContent, filename, phone)
 */

const NativePDF = (() => {

  /* ── Device metrics ────────────────────────────────────────────────────── */
  // Use the actual device pixel width (capped at 900 for readability).
  // On mobile this fills the screen; on desktop it uses A4-ish width.
  function getDeviceW(landscape) {
    const sw = window.screen.width  || 390;
    const sh = window.screen.height || 844;
    // For landscape swap axes
    const w = landscape ? Math.max(sw, sh) : Math.min(sw, sh);
    const h = landscape ? Math.min(sw, sh) : Math.max(sw, sh);
    // devicePixelRatio gives true physical pixels; cap at 1080 wide to avoid
    // enormous files, floor at 320 so tiny devices still look fine.
  ...
  /* ── Shared brand CSS — fully responsive, adapts to any screen width ──── */
  function buildCSS(landscape) {
    const { screenW } = getDeviceW(landscape);
    // Scale font sizes relative to screen width (base: 390px phone)
...
<body>${bodyHTML}</body></html>`;
  ...
  /* ── Render HTML to image sized to the device screen ───────────────────── */
  async function buildAndShare(bodyHTML, filename, phone, { landscape = false } = {}) {
    const { screenW, physW, dpr } = getDeviceW(landscape);

    // Render the iframe at actual screen CSS width so layout matches the device
    const cssW = screenW;
    const html  = buildDoc(bodyHTML, { landscape });
    const blob  = new Blob([html], { type: 'text/html' });
    const url   = URL.createObjectURL(blob);

    // Off-screen iframe sized to device screen width, tall enough for full content
    const frame = document.createElement('iframe');
    frame.style.cssText = [
      'position:fixed',
      'top:-99999px',
    ...
    // Load and wait for content + layout to settle
    await new Promise(resolve => {
      frame.onload = () => setTimeout(resolve, 500);
      frame.src = url;
    });

    // Read the true rendered height of the content
    ...
    // Give it a moment to reflow after resize
    await new Promise(r => setTimeout(r, 150));

    // Canvas at physical pixel density for sharpness
    const canvasW = physW;
    ...
    try {
      ...
      // SVG foreignObject approach — wraps the full rendered HTML
    ...
    if (imageFile) {
      ...
      showToast('Saved as PDF (image render unavailable)', 'info');
    }
  }
  ...
  function table({ headers, rows, colStyles = [], headerColor = '#28a745' }) {
    ...
    const bodyRows = rows.map(({ cells, rowClass = '' }) => {
      const tds = cells.map((cell, i) => {
        ...
        return `<td class="ta-${align[0]}" style="${style}">${h(cell)}</td>`;
      }).join('');
  ...
  /* ── Helper: colour for a balance string ───────────────────────────────── */
...
/* ═══════════════════════════════════════════════════════════════════════════
   PDF helper stubs — replicate the old jsPDF helper API surface
   ═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportEntityToPDF  — Entity / Supplier account statement
   ═══════════════════════════════════════════════════════════════════════════ */
  ...
  try {
    ...
    if (transactions.length > 0) {
      ...
      const buildTxRow = (t, runBal, isMerged = false) => {
        ...
        const bal   = Math.abs(runBal.val) < 0.01 ? 'SETTLED' : 'Rs ' + fmtAmt(Math.abs(runBal.val));
        const desc  = isMerged
      ...
      if (mergedTxns.length > 0) {
        ...
        mergedRows.push({ cells: ['','SUBTOTAL','','Rs '+fmtAmt(mTotOut),'Rs '+fmtAmt(mTotIn),''], rowClass: 'row-merged-sub' });
        txHTML += table({
      ...
      if (normalTxns.length > 0) {
        ...
        const finalBalStr = Math.abs(finalBal) < 0.01 ? 'SETTLED' : 'Rs ' + fmtAmt(Math.abs(finalBal));
        txRows.push({ cells: ['','TOTAL','','Rs '+fmtAmt(totOut),'Rs '+fmtAmt(totIn), finalBalStr], rowClass:'row-total' });
        txHTML += table({
        ...
        txHTML += `<div class="summary-box" style="background:#f5f5f5;">
          <div class="item"><span class="label">Total OUT:</span><span class="c-red fw-700">Rs ${h(fmtAmt(totOut))}</span></div>
          <div class="item"><span class="label">Total IN:</span><span class="c-green fw-700">Rs ${h(fmtAmt(totIn))}</span></div>
    ...
    if (isSupplier && supplierMaterials.length > 0) {
      ...
      matRows.push({ cells:['','TOTAL','','Rs '+fmtAmt(totInvoice),'Rs '+fmtAmt(totPaid),'Rs '+fmtAmt(totRemaining), totRemaining<=0?'CLEARED':''], rowClass:'row-total' });
      matHTML += table({
      ...
      matHTML += `<div class="summary-box" style="background:#fff5eb;">
        <div class="item"><span class="label">Total Invoiced:</span><span class="fw-700">Rs ${h(fmtAmt(totInvoice))}</span></div>
        <div class="item"><span class="label">Paid:</span><span class="c-green fw-700">Rs ${h(fmtAmt(totPaid))}</span></div>
        <div class="item"><span class="label">Outstanding:</span><span class="${totRemaining>0?'c-red':'c-gray'} fw-800">Rs ${h(fmtAmt(totRemaining))}</span></div>
      </div>`;
    }
    ...
    // Single-page equivalent: always try share first, fall back to print
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportCustomerToPDF  — Sales customer account statement
   ═══════════════════════════════════════════════════════════════════════════ */
  ...
  try {
    ...
    const buildSaleRow = async (t, runBal) => {
      ...
      if (isOldDebt) {
        ...
        detailLabel=`${fmtAmt(t.quantity||0)} kg × Rs ${fmtAmt(up)}`;
        ...
        const up = (t.unitPrice&&t.unitPrice>0)?t.unitPrice:await getEffectiveSalePriceForCustomer(t.customerName,t.supplyStore||'STORE_A');
        detailLabel=`${fmtAmt(t.quantity||0)} kg × Rs ${fmtAmt(up)}`;
        if (partial>0) detailLabel+=` | Paid: Rs ${fmtAmt(partial)} Due: Rs ${fmtAmt(val-partial)}`;
      } else if (pt==='CREDIT' && t.creditReceived) {
        const val = await getSaleTransactionValue(t);
      ...
      const balStr = Math.abs(runBal.val)<0.01 ? 'SETTLED'
        : runBal.val>0 ? 'Rs '+fmtAmt(runBal.val) : 'OVERPAID Rs '+fmtAmt(Math.abs(runBal.val));
      return { debit, credit, qty: t.quantity||0, typeLabel,
        cells: [displayDate, typeLabel, detailLabel.substring(0,55),
          debit>0?'Rs '+fmtAmt(debit):'-', credit>0?'Rs '+fmtAmt(credit):'-', balStr] };
    };
    ...
    if (transactions.length > 0) {
        ...
        const mergedRows = mergedTxns.map(t => {
          ...
          return {
            cells: [formatDisplayDate(t.date), typeLabel,
              [_pdfMergedPeriodLabel(t), _pdfMergedCountLabel(t), !isSettled?`Net due: Rs ${fmtAmt(netOut)}`:'Settled'].join(' — '),
              netOut>0?'Rs '+fmtAmt(netOut):'-', isSettled?'Rs '+fmtAmt(ms.cashSales||0):'-', balTxt],
            rowClass: 'row-merged',
          };
        });
        const mNetTotal = mergedTxns.reduce((s,t)=>s+((t.mergedSummary?.netOutstanding)||t.totalValue||0),0);
        mergedRows.push({ cells:['','SUBTOTAL',`${mergedTxns.length} year-end records`,mNetTotal>0?'Rs '+fmtAmt(mNetTotal):'-','',mNetTotal<=0.01?'SETTLED':'Rs '+fmtAmt(mNetTotal)], rowClass:'row-merged-sub' });
        txHTML += table({
      ...
      if (normalTxns.length > 0) {
        ...
        const finalBalStr = Math.abs(finalBal)<0.01?'SETTLED':finalBal>0?'DUE Rs '+fmtAmt(finalBal):'OVERPAID Rs '+fmtAmt(Math.abs(finalBal));
        txRows.push({ cells:['TOTALS','',`${fmtAmt(totQty)} kg total`,'Rs '+fmtAmt(totDebit),'Rs '+fmtAmt(totCredit),finalBalStr], rowClass:'row-total-green' });
        txHTML += table({
        ...
        txHTML += `<div class="summary-box" style="background:#f0fff4;border:1px solid #c3e6cb;">
          <div class="item"><span class="label">Total Debit:</span><span class="c-red fw-700">Rs ${h(fmtAmt(totDebit))}</span></div>
          <div class="item"><span class="label">Total Credit:</span><span class="c-green fw-700">Rs ${h(fmtAmt(totCredit))}</span></div>
    ...
    const bodyHTML = `<div class="page">
      ${docHeader(color, 'Naswar Manufacturers &amp; Dealers · Sales Tab Statement')}
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportCustomerData  — All-customers / rep-customers list (landscape)
   ═══════════════════════════════════════════════════════════════════════════ */
async function exportCustomerData(type) {
  ...
  try {
    ...
    const custRows = sorted.map(cust => {
      ...
      const netStr = Math.abs(net)<0.01?'SETTLED':net>0?'Rs '+fmtAmt(net):'OVERPAID Rs '+fmtAmt(Math.abs(net));
      return { cells:[cust.name,cust.phone,cust.address.substring(0,35),
        cust.debt>0?'Rs '+fmtAmt(cust.debt):'-', cust.paid>0?'Rs '+fmtAmt(cust.paid):'-',
        netStr, fmtAmt(cust.qty), formatDisplayDate(cust.lastDate)||'-'], rowClass:'' };
    });
    const totNetStr = Math.abs(totNet)<0.01?'SETTLED (Rs 0)':totNet>0?'Rs '+fmtAmt(totNet)+' DUE':'Rs '+fmtAmt(Math.abs(totNet))+' OVERPAID';
    custRows.push({ cells:[`TOTAL (${customerMap.size} customers)`,'','','Rs '+fmtAmt(totDebt),'Rs '+fmtAmt(totPaid),totNetStr,fmtAmt(totQty),''], rowClass:'row-total-green' });
    ...
    const bodyHTML = `<div class="page">
      ...
      <div class="summary-box" style="background:#f0fff4;font-size:8px;color:#555;">
        Customers with debt: ${cntDebtors} | Settled: ${cntSettled} | Outstanding: Rs ${h(fmtAmt(Math.max(totNet,0)))}
      </div>
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportUnifiedData  — Entities or Expenses unified view
   ═══════════════════════════════════════════════════════════════════════════ */
  ...
  try {
    ...
    if (isEntities) {
      ...
      paymentEntities.filter(e=>!e.isExpenseEntity).forEach(entity => {
        ...
        const balDisplay=Math.abs(balance)<0.01?'SETTLED':balance<0?'Rs '+fmtAmt(Math.abs(balance)):'Rs '+fmtAmt(balance);
        const balNote=Math.abs(balance)<0.01?'SETTLED':balance<0?'PAYABLE':'RECEIVABLE';
        entityRows.push({
      ...
      entityRows.push({ cells:[`TOTAL (${entityRows.length} entities)`,'','','',
        `Payable: Rs ${fmtAmt(totPayable)} / Receivable: Rs ${fmtAmt(totReceivable)}`,
      ...
      contentHTML += `<div class="summary-box" style="background:#e0f7f5;font-size:8px;">
        Total Payables: Rs ${h(fmtAmt(totPayable))} | Total Receivables: Rs ${h(fmtAmt(totReceivable))} | Net: Rs ${h(fmtAmt(Math.abs(totReceivable-totPayable)))} ${totReceivable>totPayable?'(IN OUR FAVOR)':'(NET PAYABLE)'}
      </div>`;
      ...
      if (expenses.length > 0) {
        ...
        if (mergedExp.length>0) {
          ...
          mRows.push({ cells:['','','','SUBTOTAL ('+mergedExp.length+' groups)','Rs '+fmtAmt(mTot)], rowClass:'row-merged-sub' });
          contentHTML += table({
        ...
        if (normalExp.length>0) {
          ...
          const expRows = normalExp.map(e=>({ cells:[
            ...
            (e.description||'-').substring(0,45), 'Rs '+fmtAmt(parseFloat(e.amount)||0)], rowClass:'' }));
          const totalAmt = expenses.reduce((s,e)=>s+(parseFloat(e.amount)||0),0);
          expRows.push({ cells:['','','','TOTAL ('+expenses.length+' records)','Rs '+fmtAmt(totalAmt)], rowClass:'row-total-orange' });
          contentHTML += table({
          ...
          if (Object.keys(nameGroups).length>1) {
            const bkRows = Object.entries(nameGroups).sort(([,a],[,b])=>b-a)
              .map(([name,total])=>`<div class="bk-row"><span class="bk-name">${h(name.substring(0,30))}</span><span class="bk-amt">Rs ${h(fmtAmt(total))}</span></div>`)
              .join('');
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportExpenseOverlayToPDF  — Single expense category history
   ═══════════════════════════════════════════════════════════════════════════ */
  ...
  try {
      ...
      if (mergedRecs.length>0) {
        ...
        const mRows = mergedRecs.map(e=>({
          cells:[_pdfMergedPeriodLabel(e),`${_pdfMergedCountLabel(e)} — ${(e.description||'Year-end merged total').substring(0,45)}`,
            'Rs '+fmtAmt(parseFloat(e.amount)||0),'★ MERGED'],
          rowClass:'row-merged',
        }));
        const mTot=mergedRecs.reduce((s,e)=>s+(parseFloat(e.amount)||0),0);
        mRows.push({ cells:['','SUBTOTAL ('+mergedRecs.length+' year periods)','Rs '+fmtAmt(mTot),''], rowClass:'row-merged-sub' });
        contentHTML += table({
      ...
      if (normalRecs.length>0) {
        ...
        const expRows = normalRecs.map(e=>{
          ...
          return { cells:[formatDisplayDate(e.date)||e.date||'-',(e.description||'No description').substring(0,55),
            'Rs '+fmtAmt(parseFloat(e.amount)||0),'Rs '+fmtAmt(runningTotal)], rowClass:'' };
        });
        expRows.push({ cells:['','TOTAL ('+records.length+' entries)','Rs '+fmtAmt(total),''], rowClass:'row-total-orange' });
        contentHTML += table({
    ...
    const bodyHTML = `<div class="page">
      ...
      <div class="doc-meta">
        ...
        <div class="kv"><span class="k">Total:</span><span class="v c-orange fw-700">Rs ${h(fmtAmt(total))}</span></div>
        <div class="kv"><span class="k">Generated:</span><span class="v">${h(now.toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}))} at ${h(now.toLocaleTimeString('en-US'))}</span></div>
      </div>
      ${divider(color)}
      ${contentHTML}
...
/* ═══════════════════════════════════════════════════════════════════════════
   exportRepCustomerToPDF  — Rep sales customer statement
   ═══════════════════════════════════════════════════════════════════════════ */
async function exportRepCustomerToPDF() {
  ...
  try {
    let transactions = repSales.filter(s=>s.customerName===customerName && s.salesRep===currentRepProfile);
    const now = new Date();
    if (fromVal||toVal) {
      transactions = transactions.filter(t=>{
        if (t.transactionType==='OLD_DEBT') return true;
    ...
    const buildRow = (t, runBal) => {
      ...
      runBal.val+=(debit-credit);
      const balStr=Math.abs(runBal.val)<0.01?'SETTLED':runBal.val>0?'Rs '+fmtAmt(runBal.val):'OVERPAID Rs '+fmtAmt(Math.abs(runBal.val));
    ...
    if (transactions.length>0) {
      ...
      const finalBal=totDebit-totCredit;
      const finalBalStr=Math.abs(finalBal)<0.01?'SETTLED':finalBal>0?'DUE Rs '+fmtAmt(finalBal):'OVERPAID Rs '+fmtAmt(Math.abs(finalBal));
      txRows.push({ cells:['TOTALS','',`${fmtAmt(totQty)} kg total`,'Rs '+fmtAmt(totDebit),'Rs '+fmtAmt(totCredit),finalBalStr], rowClass:'row-total' });
      txRows.forEach(r => {
        const typeTxt = r.cells[1]||'';
        // type colour applied via HTML — cells already plain text for the table renderer
      });
      txHTML += table({
        headers:['Date','Type','Details','Debit (Sale)','Credit (Rcvd)','Balance'],
        rows: txRows,
    ...
    const bodyHTML = `<div class="page">
      ${docHeader(color,'Naswar Manufacturers &amp; Dealers · Rep Sales Statement')}
